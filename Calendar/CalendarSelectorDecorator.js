...
const CalendarSelectorDecorator = hoc((config, Wrapped) => {
	const CalendarSelector = (props) => {
		...
		const {dayNameLength, locale, selected, ...rest} = calendarSelectorDecoratorProps;
		...
		const handleSelect = useCallback(({selected: selectedDay}) => {
			...
			forwardCustom('onSelect', () => ({selected: generalSelected, content}))(null, calendarSelectorDecoratorProps);
		}, [dayNameLength, calendarSelectorDecoratorProps, state]);
