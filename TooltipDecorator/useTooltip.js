...
const removeTooltipProps = ({...props}) => {
	delete props.tooltipScreenEdgeKeepout;
...
function useTooltip (props = {}) {
	const {
		tooltipScreenEdgeKeepout = (24 + 24),
	...
	useEffect(() => {
		...
		return () => {
			if (currentTooltip === clientRef.current) { // TBD: avoid currentTooltip from closure
				...
				if (mutableRefCurrent.mutationObserver) {
					mutableRefCurrent.mutationObserver.disconnect();
				}

				if (mutableRefCurrent.resizeObserver) {
	...
	const setTooltipLayout = useCallback(() => {
		...
		const overflow = calcOverflow(tooltipNode, clientNode, newState.tooltipDirection, ri.scale(tooltipScreenEdgeKeepout));
	...
	}, [tooltipText, rtl, tooltipPosition, tooltipType, tooltipScreenEdgeKeepout, layoutInfo, tooltipRef, clientRef]);
