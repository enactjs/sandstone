...
describe('onScrollStart/Stop Events behavior ', function () {
	...
	it.skip('should display Scroll Events in Action with 5-way Down and Up [GT-28470]', function () {
		...
		for (; index < 11; index++) {
			...
			Page.delay(1000);
			expect(Page.list.getAttribute('data-scrolling-events')).to.be.above(String(index));
		}
		...
		for (; index < 21; index++) {
			...
			if (index > 17) {
				...
				expect(Page.list.getAttribute('data-scrolling-events')).to.be.above(String(index - 7));
			}
		}
	});
});
