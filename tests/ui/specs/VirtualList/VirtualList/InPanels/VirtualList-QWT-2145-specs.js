...
describe('VirtualList in Panels', function () {
	...
	it('should spotlight not be truncated [QWT-2145]', async function () {
		...
		expect(await Page.textContent()).to.equal('Header Item');
		...
		expect(Number(await Page.getScrollThumbPosition())).to.above(0);
		...
		expect(((await Page.getListRect()).bottom - (await Page.activeElementRect()).top)).to.above(78);
	});
});
