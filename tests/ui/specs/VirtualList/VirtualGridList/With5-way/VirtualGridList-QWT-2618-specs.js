...
describe('Page scroll via 5-way', function () {
	...
	it('should scroll via 5-way on last item on the page [QWT-2618]', async function () {
		...
		expect(await Page.bottomRightVisibleItemId()).to.be.equal('item19');
		// Step 5: 5-way Up to the previous item.
		await Page.spotlightUp();
		await Page.delay(500);
		// Step 5-1 Verify: The list does not Scroll Down.
		expect(await Page.scrollThumbPosition()).to.be.equal(curVerticalScrollThumbPosition);
		...
		expect(await Page.bottomRightVisibleItemId()).to.be.equal('item9');
		// Step 8: 5-way Left to the previous item.
		await Page.spotlightLeft();
		await Page.delay(500);
		// Step 8-1 Verify: The list does not Scroll Left.
		expect(await Page.scrollThumbPosition()).to.be.equal(curHorizontalScrollThumbPosition);
