...
describe('Scroller', function () {
	...
	describe('FocusableScrollbar knobs', function () {
		...
		it('should focus on scrollthumb with focusableScrollbar `byEnter`[QWTC-2136]', async function () {
			...
			expect(await ScrollerPage.buttonTop.isFocused(), 'focus').to.be.true();
			...
			expect(await ScrollerPage.focusableBody.isFocused()).to.be.true();
		...
		it('should move its focus from scrollbar with directional keys when focusableScrollbar is `byEnter`', async function () {
			...
			expect(await ScrollerPage.buttonLeft.isFocused(), 'focus').to.be.true();
			await ScrollerPage.spotlightRight();
			expect(await ScrollerPage.verticalScrollThumb.isFocused()).to.be.true();
			await ScrollerPage.spotlightRight();
			expect(await ScrollerPage.buttonRight.isFocused(), 'focus').to.be.true();
			await ScrollerPage.spotlightLeft();
			expect(await ScrollerPage.verticalScrollThumb.isFocused()).to.be.true();
			await ScrollerPage.spotlightUp();
			expect(await ScrollerPage.buttonTop.isFocused(), 'focus').to.be.true();
		...
		it('should focus on scrollthumb with 5-way key and focusableScrollbar `true`[QWTC-2089]', async function () {
			...
			expect(await initialVerticalScrollTumbPosition).to.equal('0');
			...
			expect(((await ScrollerPage.getScrollThumbPosition()).vertical > initialVerticalScrollTumbPosition)).to.be.true();
			// Step 6: 5-Way Left.
			await ScrollerPage.spotlightLeft();
			...
			expect(await initialHorizontalScrollTumbPosition).to.equal('0');
			// Step 7: 5-Way Right.
			await ScrollerPage.spotlightRight();
			await ScrollerPage.delay(1000);
			// Step 7 Verify: The scroller Scrolls Right. Check HorizontalScrollThumb's position change.
			expect(((await ScrollerPage.getScrollThumbPosition()).horizontal > initialHorizontalScrollTumbPosition)).to.be.true();
			// Step 8: 5-Way Up.
			await ScrollerPage.spotlightUp();
		...
		it('should Scrolling via 5-way Key with Spotlight on the ScrollThumb [QWTC-2135]', async function () {
			...
			expect((await ScrollerPage.getScrollThumbPosition()).vertical).to.equal('0');
			// Step 8: 5-Way Left.
			await ScrollerPage.spotlightLeft();
		...
		it('Content animates with Click on scrollbar [QWTC-2386]', async function () {
			...
			expect((await ScrollerPage.getScrollThumbPosition()).vertical).to.equal('1');
	...
	describe('RTL Locale', function () {
		...
		it('should not scroll with Click on Scrollbar [QWTC-2064]', async function () {
			...
			expect(await ScrollerPage.getHorizontalScrollOffsetTop()).to.equal((await ScrollerPage.getScrollerRect()).height);
