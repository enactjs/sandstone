...
describe('Item Specs', () => {
	...
	test('should support adding a `label`', () => {
		...
		const {getByText} = render(<ItemBase label={expected}>Hello Item</ItemBase>);
		const label = getByText('Hello Item');
	...
	test('should support label with 0', () => {
		...
		const {getByText} = render(<ItemBase label={expected}>Hello Item</ItemBase>);
		const label = getByText(expected);
	...
	test('should support adding text as a child when a label is also set', () => {
		...
		const {getByText} = render(<ItemBase label="Example Label">{expected}</ItemBase>);
		const actual = getByText(expected);
	...
	test('should support `slotBefore`', () => {
		...
		const {getByText} = render(<ItemBase data-testid="item" slotBefore={expected}>Hello Item</ItemBase>);
		const actual = getByText(expected);
	...
	test('should support `slotAfter`', () => {
		...
		const {getByText} = render(<ItemBase data-testid="item" slotAfter={expected}>Hello Item</ItemBase>);
		const actual = getByText(expected);
